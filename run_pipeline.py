...
# ---------------------------------------------------------------------------
def run_oecd(results):
    print("\n=== OECD / FAO REALITY CHECK ===")
    import matplotlib.pyplot as plt
    ref = pd.read_csv(C.DATA / "oecd_reference_yields.csv")
    clean, _ = data.clean_crop_yield(data.load_crop_yield(prefer_full=True, n_rows=120_000))
    ds = clean.groupby("Crop")[C.B_TARGET].mean()
    comp = pd.DataFrame({"dataset_mean": ds}).join(ref.set_index("crop")["real_world_yield_t_ha"])
    fig, ax = plt.subplots(figsize=(9, 5))
    x = range(len(comp)); w = 0.38
    ax.bar([i - w / 2 for i in x], comp["dataset_mean"], w,
           label="Kaggle dataset (synthetic)", color="#4c72b0")
    ax.bar([i + w / 2 for i in x], comp["real_world_yield_t_ha"], w,
           label="Real-world avg (FAO/OWID)", color="#dd8452")
    ax.set_xticks(list(x)); ax.set_xticklabels(comp.index, rotation=20)
    ax.set(title="Reality check - synthetic dataset vs real-world yields", ylabel="Yield (t/ha)")
    ax.legend()
    C.savefig(fig, "D_oecd_reality_check.png")
    results["oecd"] = {"dataset_cross_crop_std": float(comp["dataset_mean"].std()),
                       "real_cross_crop_std": float(comp["real_world_yield_t_ha"].std())}
